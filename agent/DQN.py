...
class DQN():
    def __init__(self,input_channels,width,action_dim,hidden_dim,batch_size,capacity,learning_rate,gamma,e_greedy,replace_target_iter,noisy,eps_min=0.01,eps_dec=5e-7,checkpoint_dir=''):
    ...
    def learn(self):
        if self.memory.counter < self.batch_size: # 如果记忆库中的样本数小于batch_size，就不进行学习
            return
...
class DQN2():
    # 包含DDQN和DuelingDQN
    def __init__(self,input_channels,width,action_dim,hidden_dim,batch_size,capacity,learning_rate,gamma,e_greedy,replace_target_iter,noisy,DDQN=False,DuelingDQN=False,eps_min=0.01,eps_dec=5e-7,checkpoint_dir=''):
